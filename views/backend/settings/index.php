...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div class="p-6 border-b border-slate-100 flex items-center gap-3">
            ...
            <div>
                <h2 class="text-lg font-semibold text-slate-800">Tombol Saran via WhatsApp</h2>
                <p class="text-slate-500 text-sm mt-1">Tampilkan tombol mengambang untuk saran, masukan, atau aduan.</p>
            </div>
        </div>
        <form id="whatsappForm" class="p-6 space-y-4">
            ...
            <label class="inline-flex items-center cursor-pointer">
                <input type="checkbox" name="whatsapp_enabled" value="1" class="sr-only peer"
                    <?= !array_key_exists('whatsapp_enabled', $settings) || filter_var($settings['whatsapp_enabled'], FILTER_VALIDATE_BOOLEAN) ? 'checked' : '' ?>>
                <div class="relative w-11 h-6 bg-slate-200 rounded-full peer peer-focus:ring-4 peer-focus:ring-green-200 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-green-500"></div>
                <span class="ml-3 text-sm font-medium text-slate-700">Aktifkan tombol WhatsApp</span>
            </label>

            <div>
                <label class="block text-sm font-medium text-slate-700 mb-1">Nomor WhatsApp Sekolah</label>
                <input type="text" name="whatsapp_number" value="<?= e($settings['whatsapp_number'] ?? '') ?>"
                    placeholder="<?= e($profile['phone'] ?? 'Contoh: 6281234567890') ?>"
                    class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500">
                <p class="text-xs text-slate-500 mt-1">Boleh diawali 0 atau 62. Jika kosong, sistem memakai nomor telepon pada profil sekolah.</p>
            </div>

            <div>
                <label class="block text-sm font-medium text-slate-700 mb-1">Pesan Pembuka</label>
                <textarea name="whatsapp_message" rows="3"
                    class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500"
                    placeholder="Halo, saya ingin menyampaikan saran/masukan/aduan kepada pihak sekolah."><?= e($settings['whatsapp_message'] ?? '') ?></textarea>
            </div>
    ...
    <script>
        ...
        document.getElementById('whatsappForm')?.addEventListener('submit', function (e) {
            e.preventDefault();
            const formData = new FormData(this);
            if (!formData.has('whatsapp_enabled')) {
                formData.append('whatsapp_enabled', '0');
            }
            ...
            fetch('/admin/pengaturan/update', {
                ...
                if (data.success) {
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'Gagal',
                        text: data.message || 'Gagal menyimpan pengaturan WhatsApp'
                    });
                }
            });
        });
