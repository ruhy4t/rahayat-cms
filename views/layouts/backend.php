        ...
        <aside id="sidebar"
            ...
            <nav class="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 pb-8 pt-6">
                ...
                <div class="space-y-1">
                    ...
                    $canAccessSpmbSettings = in_array('spmb', $userPerms) && (($user['role'] ?? '') === 'admin' || !empty($user['is_spmb_committee']));
                    if ($canAccessSpmbSettings):
                        ?>
                        <a href="<?= $isSwasta ? '/admin/spmb' : '/admin/spmb/pengaturan' ?>"
                ...
                <div class="space-y-1">
                    ...
                    <form action="/logout" method="POST">
                        <?= Security::csrfInput() ?>
                        <button type="submit"
                            class="flex items-center w-full px-4 py-3 text-red-400 hover:bg-red-900/20 hover:text-red-300 rounded-lg transition-colors group">
